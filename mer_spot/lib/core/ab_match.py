...
from mer_spot.lib.core.utils_ab import jaccard_with_anchors
...
def match_anchor_gt(cfg, num_actions, anchor_x, anchor_w, glabel, gbbox, num_match):
    ...
    for idx in range(num_actions):
        ...
        mask = torch.gt(jaccards, match_scores) #比较a中元素大于（这里是严格大于）b中对应元素，大于则为1，不大于则为0，这里a为Tensor，b可以为与a的size相同的Tensor或常数。
...
##match_x, match_w, match_labels, match_scores 1*112


def anchor_bboxes_encode(cfg, anchors, glabels, gbboxes, g_action_nums, layer):  #真实框框编码
    '''
    ...
    temporal_length = cfg.MODEL.TEMPORAL_LENGTH[layer]  # TEMPORAL_LENGTH: [16, 8, 4, 2]
    stride = cfg.MODEL.TEMPORAL_STRIDE[layer]  # TEMPORAL_STRIDE: [8, 16, 32, 64]
    num_dbox = len(cfg.MODEL.ASPECT_RATIOS)  # ASPECT_RATIOS: [0.5, 0.75, 1, 1.5, 2]

    # anchors_class: bs, ti*n_box, nclass. others: bs, ti*n_box
    anchors_rx = anchors[:, :, -2]  #起始帧
    anchors_rw = anchors[:, :, -1]  #终点帧
    anchors_class = anchors[:, :, :cfg.DATASET.NUM_CLASSES]  # NUM_CLASSES: 2类别 只考虑宏表情微表情
    # dboxes_w, dboxes_x: ti*n_box.
    dboxes_w, dboxes_x = default_box(temporal_length, stride, cfg.MODEL.ASPECT_RATIOS)  # ASPECT_RATIOS: [0.5, 0.75, 1, 1.5, 2]
...
##batch_match_x, batch_match_w, batch_match_scores, batch_match_labels,  anchors_x, anchors_w, anchors_rx, anchors_rw
##32*112   anchors_class :32*112*2